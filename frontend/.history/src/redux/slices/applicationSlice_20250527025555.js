...
export const applyForJob = createAsyncThunk(
  "application/applyForJob",
  async (jobId, thunkAPI) => { // userId की जरूरत नहीं
    try {
      const res = await axiosInstance.post(`/applications/${jobId}/apply`); // ✅ RESTful endpoint
      return res.data;
    } catch (err) {
       return thunkAPI.rejectWithValue(
        err.response?.data?.message || "Something went wrong"
...
export const fetchApplicantsByJobId = createAsyncThunk(
  ...
  async (jobId, thunkAPI) => {
    try {
      const res = await axios.get(`/jobs/${jobId}/applicants`); // ✅ सही URL
      return res.data.applications; // सीधे array लें
    } catch (err) {
      return thunkAPI.rejectWithValue(err.response?.data?.message || err.message);
    }
  }

...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    // Add this to update liked jobs IDs
    updateLikedJobs: (state, action) => {
      if (state.user) {
        state.user.likedJobs = action.payload;
      }
    }
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getLikedJobs.fulfilled, (state, action) => {
        ...
        // Also update the IDs in user object
        if (state.user) {
      ...
      // Handle errors for like/unlike/getLikedJobs
      .addMatcher(
        isRejectedAction(likeJob, unlikeJob, getLikedJobs),
        (state, action) => {
          state.error = action.payload;
...
export const { setUser, updateLikedJobs } = authSlice.actions;
export default authSlice.reducer;

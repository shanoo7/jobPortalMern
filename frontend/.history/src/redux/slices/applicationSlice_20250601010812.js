...
// Fetch all student applications (protected)
export const fetchStudentApplications = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      ...
      const res = await axios.get('/application/student');
...
export const updateApplicationStatus = createAsyncThunk(
  ...
  async ({ appId, status }, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user?.token;
      const axios = createAxiosInstance(token);
      // console.log('Sending status update request for:', appId, status);

      const res = await axios.post(
        `/application/status/${appId}/update`,
        { status }
      );

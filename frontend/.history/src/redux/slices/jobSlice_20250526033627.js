...
export const updateJob = createAsyncThunk(
  'job/updateJob',
  async ({ id, updatedData }, { updatedData }) => {
    console.log("Dispatching updateJob for ID:", id);
    console.log("Updated data:", updatedData);
    const token = updatedData().auth.user?.token;
    const axios = createAxiosInstance(token);
    const res = await axios.patch(`/job/update/${id}`, updatedData);
...
// --- PATCH: Update job open/closed status (Recruiter only)

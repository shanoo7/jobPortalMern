...
import { Link, useSearchParams, useNavigate } from "react-router-dom";
import { useSelector, useDispatch } from "react-redux";
import { BriefcaseBusiness, Heart, PenBox, LogOut, Settings, User, Mail, GraduationCap, Building, FileText, Download, LinkIcon } from "lucide-react";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
  DropdownMenuTrigger,
...
function AccountManagementModal({ user, onClose, onUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (resumeFile) {
      ...
      formPayload.append('resumeUrl', resumeLink);
    } else if (formData.resume && !resumeFile) {
    ...
    try {
      ...
      toast.success("Profile updated successfully!");
      setIsEditing(false);
      setResumeLink(""); // Reset link after successful update
    } catch (error) {
      toast.error("Failed to update profile");
    } finally {
      setIsSubmitting(false);
  ...
  const getDirectDownloadLink = (url) => {
    ...
    if (url.includes("https://drive.google.com/uc?export=download")) {
      return url;
    }
  ...
  return (
    <div className="fixed inset-0 text-lg flex items-center justify-center bg-black bg-opacity-50 z-50 p-4" onClick={onClose}>
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="md:w-1/3 flex flex-col items-center">
              ...
              {user?.role === "student" && formData.resume && (
                <div className="mt-4 flex flex-col items-center gap-2">
                  <a
                    href={getDirectDownloadLink(formData.resume)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-blue-600 hover:underline"
                  >
                    <Download size={16} />
                    {resumeFile ? "Download New Resume" : "Download Resume"}
                  </a>
                  <a
                    href={formData.resume}
                    target="_blank"
            ...
            <div className="md:w-2/3">
              {isEditing ? (
                ...
                <div className="space-y-4">
                  {/* ... rest of the view mode remains the same */}
                </div>
              )}
            </div>
          </div>
      ...
      <Dialog open={showResumeOptions} onOpenChange={setShowResumeOptions}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add Resume from Google Drive</DialogTitle>
            <DialogDescription>
              Paste the shareable link of your resume from Google Drive
            </DialogDescription>
          </DialogHeader>
          
          <div className="space-y-4">
            <div>
              <Label htmlFor="resume-link">Google Drive Link</Label>
              <Input
                id="resume-link"
                type="url"
                value={resumeLink}
              ...
              <p className="text-xs text-gray-500 mt-1">
                Make sure the link is shareable and accessible to anyone with the link
              </p>
            </div>

...
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { getLikedJobs, likeJob, unlikeJob } from "@/redux/slices/authSlice";
import { toast } from "sonner";
...
const JobCard = ({ job }) => {
  ...
  const navigate = useNavigate();
  // const { user } = useSelector((state) => state.auth);
  const { user, likedJobs } = useSelector((state) => state.auth);
...
const isSaved =
  user?.likedJobs?.includes(job._id) ||
  likedJobs?.some((j) => j._id === job._id);

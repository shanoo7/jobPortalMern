...
import { Link, useSearchParams, useNavigate } from "react-router-dom";
import { useSelector, useDispatch } from "react-redux";
import { BriefcaseBusiness, Heart, PenBox, LogOut, Settings, User, Mail, FileText, Tag } from "lucide-react";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
  DropdownMenuTrigger,
...
function AccountManagementModal({ user, onClose, onUpdate }) {
  ...
  const handleResumeUpload = (e) => {
    ...
    if (file) {
      setResumeFile(file);
      setResumePreview(URL.createObjectURL(file));
      setResumeUrl(""); // Clear URL input if file is uploaded
    }
  };
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {user?.role === "student" && (
                    <>
                      ...
                      <div>
                        <Label htmlFor="resumeUrl">Enter Resume URL</Label>
                        <Input
                          id="resumeUrl"
                          name="resumeUrl"
                          type="url"
                        ...
                        />
                        <p className="text-xs text-gray-500 mt-1">Provide a direct download link to your resume</p>
                      </div>
                    </>
                  )}

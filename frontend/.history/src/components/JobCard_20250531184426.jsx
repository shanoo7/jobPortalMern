...
const JobCard = ({ job }) => {
  ...
  const { user, likedJobsLoaded } = useSelector((state) => state.auth);
  
  // Check if likedJobs data has been loaded
  const isLikedDataLoaded = likedJobsLoaded && user;
  
  // Safely check if job is saved
  const isSaved = isLikedDataLoaded ? user.likedJobs?.includes(job._id) : false;
      ...
      <CardContent>
        <div className="flex items-center justify-between">
          ...
          <Button 
            ...
            size="icon" 
            onClick={toggleSave}
            disabled={isSaving || !isLikedDataLoaded}
          >
            {isSaving ? (

...
//                     />
//                     <p className="text-sm text-gray-500 mt-1">Comma-separated list of your skills.</p>
//                   </div>
...
function AccountManagementModal({ user, onClose, onUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user?.role === 'student') {
      ...
      } else if (formData.resume && formData.resume.startsWith('http')) {
        // Only include resume if it's a permanent URL (not a blob)
        formPayload.append('resume', formData.resume);
      }
    }
  ...
  const handleResumeUpload = (e) => {
    ...
    if (file) {
      setResumeFile(file);
      // Don't create blob URL for preview - just store the file
    }
  };
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {user?.role === "student" && (
                    <div>
                      ...
                      />
                      {resumeFile && (
                        <p className="text-xs text-green-500 mt-1">New resume selected: {resumeFile.name}</p>
                      )}

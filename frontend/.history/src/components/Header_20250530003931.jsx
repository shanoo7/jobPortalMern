...
function AccountManagementModal({ user, onClose, onUpdate }) {
  ...
  return (
    <div
  className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4 text-lg"
  onClick={onClose}
>
  <div
    className=" rounded-xl border-2 shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto"
        ...
        <div className="md:w-2/3">
          {isEditing ? (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <Label htmlFor="fullname mb-10">Full Name</Label>
                <Input
                  id="fullname"
                  name="fullname"
            ...
            <div className="space-y-4">
              ...
              <InfoBlock icon={<FileText className="text-blue-600" size={18} />} label="Bio" value={formData.bio} />
              <InfoBlock
                icon={<Tag className="text-blue-600" size={18} />}
                label="Skills"
                value={formData.skills && formData.skills.split(',').map(s => s.trim()).join(', ')}
              />
              <div className="pt-4">

...
import { User, FileText, Mail, Phone, GraduationCap, Building } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
...
function AccountManagementModal({ user, onClose, onUpdate }) {
  ...
  console.log('Current form state:', { formData, isEditing, resumeFile }); // Debug
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsSubmitting(true);
    console.log('Form submission started', formData); // Debug
    ...
    // Handle file uploads
    if (profilePhotoFile) {
      console.log('Adding profile photo to form data'); // Debug
      formPayload.append('profilePhoto', profilePhotoFile);
    }

    if (resumeFile) {
      console.log('Adding resume file to form data:', resumeFile.name); // Debug
      formPayload.append('resume', resumeFile);
      formPayload.append('resumeOriginalName', resumeFile.name);
    }

    try {
      console.log('Submitting form data to API'); // Debug
      await onUpdate(formPayload);
      toast.success("Profile updated successfully!");
      setIsEditing(false);
      setResumeFile(null);
      ...
      console.error('Profile update failed:', error); // Debug
      toast.error("Failed to update profile");
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleProfilePhotoUpload = (e) => {
    const file = e.target.files[0];
    if (file) {
      console.log('Profile photo selected:', file.name); // Debug
      setProfilePhotoFile(file);
      const reader = new FileReader();
      reader.onload = (event) => {
        setFormData(prev => ({
  ...
  const handleResumeUpload = (e) => {
    ...
    if (!validTypes.includes(file.type)) {
      console.error('Invalid file type uploaded:', file.type); // Debug
  ...
  const getDownloadUrl = () => {
    if (!formData.resume) {
      console.warn('No resume available for download'); // Debug
      return "#";
    }

    if (formData.resume.includes("res.cloudinary.com")) {
      const filename = encodeURIComponent(formData.resumeOriginalName || "resume");
      console.log('Generating Cloudinary download URL for:', filename); // Debug
      return formData.resume.replace(
        "/upload/",
  ...
  const handleViewResume = (e) => {
    if (!formData.resume) {
      e.preventDefault();
      console.error('Attempted to view non-existent resume'); // Debug
      toast.error("Resume not available");
    } else {
      console.log('Viewing resume at:', formData.resume); // Debug
    }
  };
  ...
  const handleDownloadResume = (e) => {
    if (!formData.resume) {
      e.preventDefault();
      console.error('Attempted to download non-existent resume'); // Debug
      toast.error("Resume not available");
    } else {
      console.log('Downloading resume from:', getDownloadUrl()); // Debug
    }
  };

  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4" onClick={onClose}>
      <div className="border-2 rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white" onClick={(e) => e.stopPropagation()}>
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold flex items-center gap-2">
              <User className="text-blue-600" size={24} />

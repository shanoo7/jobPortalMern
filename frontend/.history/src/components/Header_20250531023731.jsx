...
function AccountManagementModal({ user, onClose, onUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append profile photo if changed
    if (profilePhotoFile) {
      console.log('Appending profile photo file'); // Debug log
      formPayload.append('file', profilePhotoFile);
    }

    // Append resume file if changed
    if (resumeFile) {
      console.log('Appending new resume file:', resumeFile.name); // Debug log
      formPayload.append('resume', resumeFile);
      formPayload.append('resumeOriginalName', resumeFile.name);
    } else if (formData.resume) {
      console.log('Keeping existing resume:', formData.resumeOriginalName); // Debug log
      formPayload.append('resumeUrl', formData.resume);
      formPayload.append('resumeOriginalName', formData.resumeOriginalName);
    }

    try {
      console.log('Form payload:', formPayload); // Debug log
      await onUpdate(formPayload);
      toast.success("Profile updated successfully!");
      setIsEditing(false);
      setProfilePhotoFile(null);
  ...
  const handleProfilePhotoUpload = (e) => {
    const file = e.target.files[0];
    if (file) {
      console.log('Profile photo selected:', file.name); // Debug log
      setProfilePhotoFile(file);
      const reader = new FileReader();
      reader.onload = (event) => {
        setFormData(prev => ({
  ...
  const handleResumeUpload = (e) => {
    const file = e.target.files[0];
    if (file) {
      console.log('Resume file selected:', file.name, file.type); // Debug log
      ...
      if (!validTypes.includes(file.type)) {
        console.error('Invalid file type:', file.type); // Debug log
        toast.error("Invalid file type. Please upload PDF, DOC, or DOCX files.");
        return;
      }
  ...
  const handleCancel = () => {
    console.log('Canceling edits'); // Debug log
    setIsEditing(false);
    setResumeFile(null);
    setProfilePhotoFile(null);
    setFormData({
  ...
  const getDownloadUrl = () => {
    if (!formData.resume) {
      console.warn('No resume available for download'); // Debug log
      return "";
    }
    
    // For Cloudinary URLs
    if (formData.resume.includes("res.cloudinary.com")) {
      const safeFileName = encodeURIComponent(formData.resumeOriginalName || "resume");
      console.log('Generating download URL for:', safeFileName); // Debug log
      return formData.resume.replace(
        "/upload/",
        `/upload/fl_attachment:${safeFileName}/`
      );
  ...
  // View resume handler with error checking
  const handleViewResume = (e) => {
    if (!formData.resume) {
      e.preventDefault();
      console.error('No resume URL available'); // Debug log
      toast.error("Resume not available for viewing");
      return;
    }
    console.log('Viewing resume at:', formData.resume); // Debug log
  };

  // Download resume handler with error checking
  const handleDownloadResume = (e) => {
    if (!formData.resume) {
      e.preventDefault();
      console.error('No resume URL available for download'); // Debug log
      toast.error("Resume not available for download");
      return;
    }
    console.log('Downloading resume from:', getDownloadUrl()); // Debug log
  };
  ...
  return (
    <div className="fixed inset-0 text-lg flex items-center justify-center bg-black bg-opacity-50 z-50 p-4" onClick={onClose}>
      <div className="border-2 rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-6">
          {/* ... (rest of your modal header code remains the same) ... */}

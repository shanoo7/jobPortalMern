...
const JobCard = ({ job }) => {
  ...
  const toggleSave = async () => {
    if (!user) {
      // Store the pending action in localStorage
    ...
    try {
      ...
      const errorMessage = error?.payload?.message || error?.message || "Failed to update saved jobs";
      toast.error(errorMessage);
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <Card className="rounded-2xl shadow-md hover:shadow-lg transition-all duration-200 border p-4 flex flex-col justify-between">
      <CardContent>
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold">{job.title}</h2>
          <Button variant="ghost" size="icon" onClick={toggleSave} disabled={isSaving}>
            {isSaving ? (

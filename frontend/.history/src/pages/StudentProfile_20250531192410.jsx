      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {studentApplications.map((app) => (
            ...
            <div onClick={() => navigate(`/job/${app.job._id}`)} key={app._id} className="border cursor-pointer rounded-lg p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  {app.job ? (
                    ...
                    <>
                      <h3 className="text-xl font-semibold text-gray-500">Job No Longer Available</h3>
                      <p className="text-gray-400 mt-1">This position has been removed by the employer</p>
                    </>
                  )}
                </div>

                <div className="text-right">
                  <p className={`capitalize ${app.status === 'accepted' ? 'text-green-600' :

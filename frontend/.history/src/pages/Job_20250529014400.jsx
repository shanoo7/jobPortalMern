...
// // JobPage.jsx में
// console.log("start")
// console.log("Backend से आया जॉब डेटा:", job);
// console.log("end")
// // applications array में हर application के applicant में पूरा डेटा होना चाहिए
//   useEffect(() => {
...
//   return (
//     <div className="text-center p-8 text-red-600 bg-red-50 rounded shadow">
//       Error: {error.message || "Failed to load job"} {/* ← Access message property */}
//     </div>
//   );
// }
...
//               {console.log("user data is comming when appling job",job.applications)}
...
//           {!isRecruiter &&  <ApplyJob job={job} user={user} applied={hasApplied} /> }
//           {/* {!isRecruiter ? (
...
//                   const applicant = application.applicant?.id ? application.applicant : user;
...
//             </div>
//           ) : (
//             <div className="rounded-lg shadow p-6 text-center text-gray-500">
...
export default function JobPage() {
  ...
  const isRecruiter = useMemo(() => {
    return (
      user?.role === "recruiter" &&
      (job?.created_by?._id === user?._id || job?.created_by === user?._id)
    );
  }, [job?.created_by, user?._id, user?.role]);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Job Description */}
        <div className="lg:col-span-2 shadow rounded-lg p-6 space-y-6">

...
const uploadLogoToCloudinary = async (logo) => {
  if (!logo) return null;
  ...
  try {
    ...
    const directUrl = extractDirectImageUrl(logo);
    ...
    if (directUrl.startsWith('http')) {
      return (await cloudinary.uploader.upload(directUrl, uploadOptions)).secure_url;
    ...
    return logo; // Return original if upload fails
  }
};
...
export const postJob = async (req, res) => {
  try {
    ...
    if (logo) {
      cloudinaryLogo = await uploadLogoToCloudinary(logo);
    }
...
export const updateJob = async (req, res) => {
  try {
    ...
    // Process logo if updated
    if (updates.logo) {
      updates.logo = await uploadLogoToCloudinary(updates.logo);
    }

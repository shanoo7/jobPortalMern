...
//     const { fullname, email, phoneNumber, bio, skills, resumeUrl, resumeOriginalName } = req.body;
...
export const updateProfile = async (req, res) => {
  try {
    ...
    // Handle both multipart form and JSON data
    const bodyData = req.files ? req.body : req.body;
    ...
    // Update all fields including resumeUrl
    ...
    fieldsToUpdate.forEach(field => {
      if (bodyData[field] !== undefined) {
        if (field === 'skills') {
          user.profile.skills = bodyData[field]
            .split(',')
            .map(skill => skill.trim())
            .filter(Boolean);
        } else if (field === 'resumeUrl' || field === 'resumeOriginalName') {
          user.profile[field] = bodyData[field];
        } else {

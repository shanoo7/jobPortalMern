...
export const updateProfile = async (req, res) => {
  try {
    ...
    if (profilePhotoFile) {
      const fileUri = getDataUri(profilePhotoFile);
      const photoUpload = await cloudinary.uploader.upload(fileUri.content, {
    ...
    if (resumeFile) {
      ...
      const sanitizedFileName = resumeFile.originalname.replace(/\.[^/.]+$/, ''); // remove extension
      const timestamp = Date.now();

      const resumeUpload = await cloudinary.uploader.upload(resumeUri.content, {
        resource_type:"auto",
        folder: "resumes",
        public_id: `resumes/${userId}_${timestamp}`
      });
